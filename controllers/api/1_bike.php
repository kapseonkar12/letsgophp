...
/*
	public function executeTATAgiAPI($POST) {
		...
		$make = $POST->requestPara->vh_make;
		$model = $POST->requestPara->vh_model;
		$variant = $POST->requestPara->vh_varient;
		...
		$mobile_no="";
		$email_id="";
		$customer_type="Individual"; // Individual or Organization
		$product_code="3122";        //For 2W   Car:3121  ComerVehicle : 3124 
		$product_name="Two Wheeler";  // For @w
		$subproduct_code="37";
		$subproduct_name="Two Wheeler";
		
		$covertype_code="1";// 1-  for  Package  ---- 2 - for  Liability
		if($covertype_code == "1")
		...
		//1-  for  New Business  2 - for  Roll Over
		$btype_code="1";
		if($btype_code=="1")
		...
		/**
		* YYYYMMDD Start date (current date when user purchase ) and Enddate shouls be after 5yr 
		...
		*/
		...
		// Aftert 5 yrs calculated for Business what we selected

		$dateString = date('Ymd', strtotime('+5 years'));
		$t = strtotime($dateString);
		$t2 = strtotime('-1 days', $t);
		
		$risk_enddate=date('Ymd', $t2) . PHP_EOL;
		$purchase_date=$risk_startdate ;

		$veh_age="";  //Calculated age from Purchase date to Current date
		/**
		* make_code : From Make Master ( Pass MANUFACTURERCODE)based on selected vehicle Model"
		* make_name : From Make Master (Pass MANUFACTURER) based on selected vehicle Model
		* model_code :From Make Master (Pass NUM_PARENT_MODEL_CODE) based on selected vehicle Model"
		* model_name : From Make Master (Pass VEHICLEMODEL)based on selected vehicle Model
 		* variant_code : From Make Master (Pass VEHICLEMODELCODE)based on selected vehicle Model"
		* variant_name : From Make Master (Pass TXT_VARIANT) based on selected vehicle Model
		* model_parent_code : From Make Master (Pass NUM_PARENT_MODEL_CODE) based on selected vehicle Model
		*
		*/
		$manf_year="";
		$make_code=$make;
		...
		/* Pass TXT_RTO_LOCATION_CODE from RTO Master(GC)or Registration number
		*  1-Registration number 2 ( eg: MH-23)
		*/
		$rto_loc_code =""; 
		...
		/* 
		*	If select driver_declaration is ODD01  Hard Coded(""opted"":""Y"")
		*      User selected values(""tenure"":""1"") 
		*	Else
		*   Hard Coded(""opted"":""N"")
